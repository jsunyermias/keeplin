...
| [0009 — Review governance is evaluated from the default branch](0009-governance-evaluated-from-the-default-branch.md) | proposed | cross-repo | Extends 0008. Addresses F-025 from the round-9 review of [keeplin#198](https://github.com/jsunyermias/keeplin/pull/198): the governance gate runs inside the head-controlled workflow, so the evaluator has no evidence it ran. Unimplemented while proposed |
