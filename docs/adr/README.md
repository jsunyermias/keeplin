...
| [0015 — Self-authorized disposal with an auditable directive](0015-self-authorized-disposal-with-an-auditable-directive.md) | accepted | cross-repo | [keeplin#206](https://github.com/jsunyermias/keeplin/issues/206). Amends the authorization precondition of 0008, which 0013 applied to genesis: the author may authorize disposal on their own pull request when the directive is machine-readable, bound and attributable. Decides that mechanism and the guard on its single-principal premise. Proposed without recommending between Option A (unconditional, guard off the authorization path) and Option C (conditional, checked at authorization time); **accepted with Option C**, so A is recorded as the rejected alternative. Measured on [keeplin-srv#114](https://github.com/jsunyermias/keeplin-srv/pull/114), where fourteen verified findings plus `GENESIS` were undisposable |
| [0016 — Refuse pre-release filesystem formats instead of silently relabelling them](0016-refuse-pre-release-filesystem-formats.md) | accepted | keeplin | [keeplin#228](https://github.com/jsunyermias/keeplin/pull/228); [keeplin#162](https://github.com/jsunyermias/keeplin/issues/162); unblocks [keeplin#207](https://github.com/jsunyermias/keeplin/issues/207). Narrowly amends 0003 for pre-release `FsBackend` layouts: refuse older stamps without mutation until the first tagged published artifact, then require real migrations prospectively; **accepted with Option C**, so A and B are recorded as the rejected alternatives |
