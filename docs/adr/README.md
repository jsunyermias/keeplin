...
| [0015 — Self-authorized disposal with an auditable directive](0015-self-authorized-disposal-with-an-auditable-directive.md) | proposed | cross-repo | [keeplin#206](https://github.com/jsunyermias/keeplin/issues/206). Amends the authorization precondition of 0008, which 0013 applied to genesis: the author may authorize disposal on their own pull request when the directive is machine-readable, bound and attributable. Decides that mechanism and the guard on its single-principal premise, and deliberately leaves the policy choice between Option A (unconditional, guard off the authorization path) and Option C (conditional, checked at authorization time) to the maintainer. Measured on [keeplin-srv#114](https://github.com/jsunyermias/keeplin-srv/pull/114), where fourteen verified findings plus `GENESIS` were undisposable |
