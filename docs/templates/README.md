...
|----------------------|----------|----------|
| Any `.rs` file with logic — library/daemon module, binary, build script, **or an integration/unit test file** (`tests/*.rs`; each `#[test] fn` is a block) | [`source-module.md`](source-module.md) (v2.4.0, mechanically verified) | next to the `.rs`, same basename |
...
| A high-risk architectural decision with alternatives and approval state | [`docs/adr/0000-template.md`](../adr/0000-template.md) | `docs/adr/NNNN-short-title.md` in the repository that owns the decision |
