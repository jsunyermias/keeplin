...
| `GET /events`, `POST /events/import`, `GET /events/:uid/export`, `DELETE /events/:uid` | **iCalendar VEVENT interop**, same shape (`text/calendar`); import stores **every** `VEVENT` in the body and returns the stored list |
| `POST /todos/import` | import **every** iCalendar `VTODO` in the body as Keeplin **to-do notes** (to-dos are notes, not resources); returns the created notes |
